...
| «Сверхновая»: ореол, спутники цвета категорий, пламя (`flame.gdshader`) | код + шейдер | Слои по билду (`balance.json → supernova`) | 🟨 |
...
|---|---|---|---|
| `floor_{a,b,c,d}.png` + `_n` | 512×512 бесшовная | 4 на биом (`world/<biome>/floor_1..4`: брусчатка · земля с мхом и корнями · клёпаная сталь с ржавчиной), тон главы — modulate | 🟨 (генератор) |
| `prop_<name>.png` + `_n` + полигон `LightOccluder2D` | 96–384 | 2 на биом: плита/обломки · пень/валун · ящик/контейнер (окклюдер — прямоугольник) | 🟨 (генератор) |
| `biolum_mushrooms_{1,2}.png`, `biolum_rune.png` + светлячки (частицы) | 48–64 | общие, цвет главы | 🟨 (генератор) |
...
| `hub_diorama_ch{1,2,3}.png`: «Затопленный город» · «Спящий лес» (стволы, кроны, светлячки, пруд) · «Ржавый порт» (склады, контейнеры, краны, корабль, причал); проявление — шейдер `hub_reveal`, тир 10 — дневная палитра | 780×1100 | 🟨 (генератор) |
...
`ember.png`, `ash.png`, `spark.png` — 32px, белые под цвет частиц (генератор). Дисторсия Взрыва и растворение врагов — шейдерами (`shockwave`, `enemy_body`), шумовые текстуры не нужны. Статус: 🟨.

## 9. Аудио (`src/assets/audio/`) — DS §06

Звуки — `tools/audio/synth_sfx.py` (процедурный синтез, воспроизводимый; numpy + scipy + ffmpeg/libvorbis),
таблица событий — `configs/audio.json`. 21 SFX + 3 музыкальных лупа + стингер. Статус: ✅ V1.

| Параметр | Значение |
|---|---|
| Формат | Ogg Vorbis q6, 44.1 кГц, стерео |
| Громкость SFX | активный RMS -16 dBFS (смесь — `volume_db` в `configs/audio.json`) |
| Громкость музыки | -18 LUFS интегрально; стингер `reward_resolve` -16 LUFS |
| Пик | true-peak ≤ -1 dBTP (после декодирования ≤ -0.9) |
| Щелчки | фейды 3 / 15 мс, релиз у каждой ноты, хвост ревера обрезан на -60 дБ |
| Лупы | бесшовные (хвост ревера завёрнут в начало); `run_base` и `run_drums` — ровно 40.00 с |
| Тональность | ре: хаб — ре мажор (72 BPM, 16 тактов ≈ 53 с), забег — ре минор (96 BPM, A/B по 8 тактов) |

Звуковой язык DS §06: искра — FM-колокольчик ре6 (серия +1 полутон — движок), топливо и Взрыв — вдох → низкий бум
+ стеклянный хвост, враг — короткое «пшш», урон — глухой удар, HP < 25% — «тук-тук» + дыхание, левел-ап — ре мажор
+ «флип» бумаги, тир Маяка — глиссандо + хор + колокол, покупка — уникальный «хрустальный» аккорд (только IAP),
кнопка — мягкий клик, Disabled — глухой «тук». Замена звука — подменить `.ogg` с тем же именем.
