...
var SKILLS_DB: Dictionary = {
	...
	"shadow_burn": {
		...
		"bg_color": Color(0.605, 0.049, 0.057, 0.75) # Красный (Атака)
	}
}
...
func add_sparks(amount: int) -> void:
	...
	if run_sparks_counter >= SKILL_CHOICE_TRIGGERED_TRASHHOLD:
		run_sparks_counter -= SKILL_CHOICE_TRIGGERED_TRASHHOLD
